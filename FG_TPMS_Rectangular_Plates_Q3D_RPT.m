%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Title: Isogeometric analysis for Structural analyses of FG-TPMS plates with 4-Dof Quasi-3D refined plate theory (Q3D-RPT) %%
...
Plate.theory.shear_func = "Hype_1";
% Plate.theory.shear_func = "Trig_1";
Plate.theory.iterated_order = 3;

% === Iterated thickness-stretching deformation function ===
...
IGA.NURBS.ref = 15; % Number of mesh refinement
...
Plate.por_dis.RD_max = 0.5;
Plate.por_dis.RD_0 = 0.6;  % RD_min = RD_max*(1- RD_0);
...
%% ======================== Material matrices =============================
[Plate.mat_mat.D] = cal_Material_Matrices_FG_TPMS_Q3D_RPT(Plate);
...
% === IGA properties ===
IGA.params.ndof   = 4;                                                           % Number of dofs of a control point
...
IGA.result.K = cal_Stiffness_Matrices_Q3D_RPT(IGA,Plate);                    % Stiffness
switch Plate.prob.type
    case 1  % Static bending
        IGA.result.F = cal_Load_Vector_Uniform_Q3D_RPT(IGA,Plate);           % Extenal load
end

% === Imposing boundary conditions ===
[IGA.params.bcdof, IGA.params.bcval] = cal_bcdof_Q3D_RPT(IGA,Plate);
...
cal_plot_defl_Q3D_RPT(IGA,Plate,'cen_defl');
% cal_plot_defl_Q3D_RPT(IGA,Plate,'NURBS_defl');

% === Displacement field ===
% Options: point, point_dist, FE_surf, FE_vol || ['u', 'v', 'w']
% cal_plot_disp_Q3D_RPT(IGA,Plate,'point',[1/2,1/4,-1/2]);
% cal_plot_disp_Q3D_RPT(IGA,Plate,'point_dist',[0/2,1/2],'u');
% cal_plot_disp_Q3D_RPT(IGA,Plate,'FE_surf','u');
% cal_plot_disp_Q3D_RPT(IGA,Plate,'FE_vol','u');

% === Strain field ===
% Options: point, point_dist, FE_surf, FE_vol || ['xx', 'yy', 'xy', 'yz', 'zx', 'zz']
% cal_plot_strain_Q3D_RPT(IGA,Plate,'point',[1/2,1/4,-1/2]);
% cal_plot_strain_Q3D_RPT(IGA,Plate,'point_dist',[1/2,1/2],'zz');
% cal_plot_strain_Q3D_RPT(IGA,Plate,'FE_surf','yy');
% cal_plot_strain_Q3D_RPT(IGA,Plate,'FE_vol','xx');

% === Stress field ===
% Options: point, point_dist, FE_surf, FE_vol || ['xx', 'yy', 'xy', 'yz', 'zx', 'zz']
% cal_plot_stress_Q3D_RPT(IGA,Plate,'point',[1/2,1/2,1/2]);
% cal_plot_stress_Q3D_RPT(IGA,Plate,'point',[1/2,1/2,0]);
% cal_plot_stress_Q3D_RPT(IGA,Plate,'point_dist',[1/2,1/2],'zz');
cal_plot_stress_Q3D_RPT(IGA,Plate,'point_dist',[0/2,1/2],'zx');
% cal_plot_stress_Q3D_RPT(IGA,Plate,'FE_surf','yy');
% cal_plot_stress_Q3D_RPT(IGA,Plate,'FE_vol','xx');
