function [K] = cal_Stiffness_Matrices_2D_FSDPT(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate stiffness matrices with 5-Dof 2D RM's FSDPT plate models %%%
    ...
    if abs(uKnot(ni)-uKnot(ni+1)) > tol && abs(vKnot(nj)-vKnot(nj+1)) > tol  % Check if the current element has nonzero area in the parametric domain
        ...
        for iGauss = 1: nGauss  % Loop over the integration points
            for jGauss = 1: nGauss
                ...
                [N, ~, ~, dNdx, ~, detJ1] = cal_Kine_Shape_2D_2nd(IGA.NURBS,ni,nj,gpt_xi,gpt_eta);
