function [K] = cal_Stiffness_Matrices_Q3D_GPT(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate stiffness matrices with 6-Dof Quasi-3D GPT plate models %%%
...
Ds = Plate.mat_mat.D.Ds;
Dbst = Plate.mat_mat.D.Dbst;
Dst = Plate.mat_mat.D.Dst;
    ...
    if abs(uKnot(ni)-uKnot(ni+1)) > tol && abs(vKnot(nj)-vKnot(nj+1)) > tol  % Check if the current element has nonzero area in the parametric domain
        ...
        for iGauss = 1: nGauss  % Loop over the integration points
            for jGauss = 1: nGauss
                ...
                for i = 1:2
                    for j = 1:2
                        ks = ks + Bs{i}'*Ds{i,j}*Bs{j}*gwt*detJ1;
                    end
                end
                ...
                kbst = 0;
                for i = 1:3
                    kbst = kbst + Bb{i}'*Dbst{i}*Bst*gwt*detJ1;
                end

                % Thickness-stretching stiffness matrix
                kst = Bst'*Dst*Bst*gwt*detJ1;
                
                Ke = Ke + kb + ks + kbst + kbst' + kst;
            end
        end
