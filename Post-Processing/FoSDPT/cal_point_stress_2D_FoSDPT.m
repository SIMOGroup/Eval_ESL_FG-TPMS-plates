function [point_stress] = cal_point_stress_2D_FoSDPT(IGA,Plate,phys_coord)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate stress field of specific point of rectangular plate with 7-Dof 2D SS's FoSDPT plate models %%%
...
h_e = [1, 4/3*(phys_coord(3)^3)/h^2, phys_coord(3) - 4/3*(phys_coord(3)^3)/h^2, phys_coord(3)^2 + 2*(phys_coord(3)^4)/h^2];
h_s = [(1 - 4*(phys_coord(3)^2)/h^2), 2*phys_coord(3)*(1 - 4*(phys_coord(3)^2)/h^2)];

% --- Material constitutive model ---
[Q] = cal_point_constitutive_FG_TPMS_2D(Plate, phys_coord(3));
...
stress_s = zeros(2,1);
for k = 1:2
    stress_s = stress_s + h_s(k)*Q.Qs*Bs{k}*U(sctrF,1);
end

point_stress = [stress_b; stress_s; 0];
