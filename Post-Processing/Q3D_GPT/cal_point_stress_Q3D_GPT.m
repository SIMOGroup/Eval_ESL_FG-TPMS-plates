function [point_stress] = cal_point_stress_Q3D_GPT(IGA,Plate,phys_coord)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate stress field of specific point of rectangular plate with 6-Dof Quasi-3D GPT plate models %%%
...
Bsst = zeros(2,ndof*nn);  % Shear-stretching
Bsst(1,6:ndof:ndof*nn) = dNdx(:,2)';
Bsst(2,6:ndof:ndof*nn) = dNdx(:,1)';

Bs = {Bps, Bsst};

% Bending-stretching stiffness matrix
Bst = zeros(1,ndof*nn);
Bst(1,6:ndof:ndof*nn) = N';
...
[gz, dgz] = compute_iterated_stretching_deformation_function(phys_coord(3),h,shear_func,stretch_func,n_iterated);
h_s = [dfz, gz];

% --- Material constitutive model ---
[Q] = cal_point_constitutive_FG_TPMS_3D(Plate, phys_coord(3));
...
point_stress = [stress_b; stress_s; stress_st];
