function [bcdof, bcval] = cal_bcdof_Q3D_GPT(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate dofs of boundary conditions with 6-Dof Quasi-3D GPT plate models %%%
...
%% ===== Dofs of control points on boundaries (dofs = u_{0}, v_{0}, w_{0}, \beta_{y}, \beta_{x}, \beta_{z}) =====
for i_bc = 1:length(bc_case)
    switch bc_case(i_bc)
        case 1  % Clamped (C)
            for idof = [1, 2, 3, 4, 5, 6]  % Constraint u_{0}, v_{0}, w_{0}, \beta_{y}, \beta_{x}, \beta_{z} at perimeter
