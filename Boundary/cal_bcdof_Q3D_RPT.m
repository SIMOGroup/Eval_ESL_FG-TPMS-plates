function [bcdof, bcval] = cal_bcdof_Q3D_RPT(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate dofs of boundary conditions with 4-Dof Quasi-3D RPT plate models %%%
...
%% ===== Dofs of control points on boundaries (dofs = u_{0}, v_{0}, w_{b}, w_{s}) =====
for i_bc = 1:length(bc_case)
    switch bc_case(i_bc)
        case 1  % Clamped (C)
            for idof = [1, 2, 3, 4]  % Constraint u_{0}, v_{0}, w_{b}, w_{s} at perimeter
                bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
            end

            for idof = [3, 4]  % Constraint dw_{b}dx, dw_{b}dy, dw_{s}dx, dw_{s}dy at perimeter (with a fine mesh)
                bcdof = [bcdof ndof*(CP_peri_2{i_bc}-1)+idof];
            end
        case 2  % Hinged (freely movable) supported
            for idof = [3, 4]  % Constraint w_{b}, w_{s} at perimeter
                bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
            end
        ...
        case 4  % Soft (quasi-movable) supported (S2)
            ...
            if i_bc == 2 || i_bc == 4
                for idof = [1]  % Constraint u_{0} at perimeter on Upper and Lower
                    bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
                end
            elseif i_bc == 1 || i_bc == 3
                for idof = [2]  % Constraint v_{0} at perimeter on Left and Right
                    bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
                end
            end
        case 5  % Hard (immovable) supported (S3)
            ...
            for idof = [1, 2]  % Constraint u_{0}, v_{0} at all perimeters
                bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
            end
    end
end
