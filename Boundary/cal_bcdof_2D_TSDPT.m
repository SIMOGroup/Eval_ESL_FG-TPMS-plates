function [bcdof, bcval] = cal_bcdof_2D_TSDPT(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate dofs of boundary conditions with 5-Dof 2D Reddy's TSDPT plate models %%%
...
%% ===== Dofs of control points on boundaries (dofs = u_{0}, v_{0}, w_{0}, \beta_{y}, \beta_{x}) =====
for i_bc = 1:length(bc_case)
    switch bc_case(i_bc)
        case 1  % Clamped (C)
            for idof = [1, 2, 3, 4, 5]  % Constraint u_{0}, v_{0}, w_{0}, \beta_{y}, \beta_{x} at perimeter
        ...
        case 4  % Soft (quasi-movable) supported (S2)
            ...
            if i_bc == 2 || i_bc == 4
                for idof = [1, 4]  % Constraint u_{0}, \beta_{y} at perimeter on Upper and Lower
                    bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
                end
            elseif i_bc == 1 || i_bc == 3
                for idof = [2, 5]  % Constraint v_{0}, \beta_{x} at perimeter on Left and Right
                    bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
                end
            end
        case 5  % Hard (immovable) supported (S3)
            ...
            for idof = [1, 2, 4, 5]  % Constraint u_{0}, v_{0}, \beta_{y}, \beta_{x} at all perimeters
                bcdof = [bcdof ndof*(CP_peri_1{i_bc}-1)+idof];
            end
    end
end
