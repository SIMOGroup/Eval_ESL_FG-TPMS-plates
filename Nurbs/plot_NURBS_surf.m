...
if note_physical_element
    for i_ele = 1:size(Iee,1)
        ...
        text(cen_coord(1), cen_coord(2), cen_coord(3), ["e"+num2str(i_ele)]);
    end
end

if note_physical_edge
    for i_edge = 1:size(Ine,1)
        text(mean(Ine(i_edge,:,1)), mean(Ine(i_edge,:,2)), mean(Ine(i_edge,:,3)), ["ed"+num2str(i_edge)], 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
    end
end
