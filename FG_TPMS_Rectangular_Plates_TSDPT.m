%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Title: Isogeometric analysis for Structural analyses of FG-TPMS plates with 5-Dof 2D Reddy Third-order shear deformation plate theory (Reddy's TSDPT) %%
...
%% ======================== Material matrices =============================
[Plate.mat_mat.D] = cal_Material_Matrices_FG_TPMS_2D_TSDPT(Plate);
...
IGA.result.K = cal_Stiffness_Matrices_2D_TSDPT(IGA,Plate);                    % Stiffness
switch Plate.prob.type
    case 1  % Static bending
        IGA.result.F = cal_Load_Vector_Uniform_2D_TSDPT(IGA,Plate);           % Extenal load
end

% === Imposing boundary conditions ===
[IGA.params.bcdof, IGA.params.bcval] = cal_bcdof_2D_TSDPT(IGA,Plate);
...
% cal_plot_defl_2D_TSDPT(IGA,Plate,'cen_defl');
% cal_plot_defl_2D_TSDPT(IGA,Plate,'NURBS_defl');

% === Displacement field ===
% Options: point, point_dist, FE_surf, FE_vol || ['u', 'v', 'w']
% cal_plot_disp_2D_TSDPT(IGA,Plate,'point',[1/2,1/4,-1/2]);
% cal_plot_disp_2D_TSDPT(IGA,Plate,'point_dist',[0/2,1/2],'u');
% cal_plot_disp_2D_TSDPT(IGA,Plate,'FE_surf','u');
% cal_plot_disp_2D_TSDPT(IGA,Plate,'FE_vol','u');

% === Strain field ===
% Options: point, point_dist, FE_surf, FE_vol || ['xx', 'yy', 'xy', 'yz', 'zx', 'zz']
% cal_plot_strain_2D_TSDPT(IGA,Plate,'point',[1/2,1/4,-1/2]);
% cal_plot_strain_2D_TSDPT(IGA,Plate,'point_dist',[0/2,1/2],'zx');
% cal_plot_strain_2D_TSDPT(IGA,Plate,'FE_surf','yy');
% cal_plot_strain_2D_TSDPT(IGA,Plate,'FE_vol','xx');

% === Stress field ===
% Options: point, point_dist, FE_surf, FE_vol || ['xx', 'yy', 'xy', 'yz', 'zx', 'zz']
% cal_plot_stress_2D_TSDPT(IGA,Plate,'point',[1/2,1/2,1/2]);
% cal_plot_stress_2D_TSDPT(IGA,Plate,'point',[0/2,1/2,0]);
% cal_plot_stress_2D_TSDPT(IGA,Plate,'point_dist',[1/2,1/2],'xx');
% cal_plot_stress_2D_TSDPT(IGA,Plate,'point_dist',[0/2,1/2],'zx');
% cal_plot_stress_2D_TSDPT(IGA,Plate,'FE_surf','yy');
% cal_plot_stress_2D_TSDPT(IGA,Plate,'FE_vol','xx');
